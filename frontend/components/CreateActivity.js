...
class CreateActivity extends Component {
  ...
  uploadFile = async e => {
    ...
    data.append('upload_preset', 'jeepney');

    const res = await fetch('https://api.cloudinary.com/v1_1/didhtdmyv/image/upload', {
      method: 'POST',
      body: data
    });
  ...
  render() {
    return (
      <Mutation mutation={CREATE_ACTIVITY_MUTATION} variables={this.state}>
        {(createActivity, { loading, error }) => (
          <Form onSubmit={async e => {
            e.preventDefault();
            const res = await createActivity();
            Router.push({
              pathname: '/activity',
              query: { id: res.data.createActivity.id }
            })
          }}>
            <Error error={error} />
...
export { CREATE_ACTIVITY_MUTATION };
